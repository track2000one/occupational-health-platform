...
async function apiError(response: Response) {
  try {
    ...
    // Use the HTTP status below if the response is not JSON.
  }
    ...
    return <Stack spacing={2.5}>
      {canEdit && <Paper sx={{ p: 2.5 }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(4,1fr)' }, gap: 1.5 }}>
          <TextField label={isRtl ? 'الاسم' : 'Name'} value={memberForm.name} onChange={event => setMemberForm(value => ({ ...value, name: event.target.value }))} />
          <TextField label={isRtl ? 'الرقم الوظيفي' : 'Employee no.'} value={memberForm.employee_number} onChange={event => setMemberForm(value => ({ ...value, employee_number: event.target.value }))} />
          <TextField label={isRtl ? 'المسمى الوظيفي' : 'Job title'} value={memberForm.job_title} onChange={event => setMemberForm(value => ({ ...value, job_title: event.target.value }))} />
          <TextField label={isRtl ? 'المؤهل' : 'Qualification'} value={memberForm.qualification} onChange={event => setMemberForm(value => ({ ...value, qualification: event.target.value }))} />
        </Box>
      ...
      {rows.length === 0 ? <EmptyState text={isRtl ? 'لا توجد أسماء مسجلة.' : 'No workforce members.'} /> : rows.map(member => <Paper key={member.id} sx={{ p: 2.5 }}>
        ...
        <TableContainer sx={{ mt: 2 }}><Table size="small"><TableHead><TableRow>
      ...
      {canEdit && rows.length > 0 && <Paper sx={{ p: 2.5 }}>
        ...
        <Box sx={{ mt: 1.5, display: 'grid', gridTemplateColumns: { xs: '1fr', md: '2fr 2fr repeat(5,1fr)' }, gap: 1.2 }}>
          <TextField select label={isRtl ? 'الموظف' : 'Member'} value={targetForm.member} onChange={event => setTargetForm(value => ({ ...value, member: event.target.value }))}>{rows.map(item => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}</TextField>
          <TextField label={isRtl ? 'البند' : 'Item'} value={targetForm.item} onChange={event => setTargetForm(value => ({ ...value, item: event.target.value }))} />
          {(['annual_target','q1_target','q2_target','q3_target','q4_target'] as const).map(field => <TextField key={field} type="number" label={field === 'annual_target' ? (isRtl ? 'سنوي' : 'Annual') : field.replace('_target','').toUpperCase()} value={targetForm[field]} onChange={event => setTargetForm(value => ({ ...value, [field]: event.target.value }))} />)}
        </Box>
  ...
  return <Box dir={isRtl ? 'rtl' : 'ltr'}>
    <Stack direction={{ xs: 'column', lg: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', lg: 'center' }} spacing={2} sx={{ mb: 2.5 }}>
      ...
      <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
        <TextField size="small" type="number" label={isRtl ? 'السنة' : 'Year'} value={year} onChange={event => setYear(Number(event.target.value || now.getFullYear()))} sx={{ width: 110 }} />
        <TextField size="small" select label={isRtl ? 'الشهر' : 'Month'} value={month} onChange={event => setMonth(Number(event.target.value))} sx={{ minWidth: 130 }}>{monthNames.map((name, index) => <MenuItem key={name} value={index + 1}>{name}</MenuItem>)}</TextField>
        <TextField size="small" select label={isRtl ? 'الربع' : 'Quarter'} value={quarter} onChange={event => setQuarter(Number(event.target.value))} sx={{ minWidth: 100 }}>{[1,2,3,4].map(value => <MenuItem key={value} value={value}>Q{value}</MenuItem>)}</TextField>
        <Button variant="outlined" startIcon={<RefreshIcon />} onClick={() => setRefreshVersion(value => value + 1)} disabled={loading}>{isRtl ? 'تحديث' : 'Refresh'}</Button>
      </Stack>
    </Stack>
    ...
    {tab === 0 && summary && <Stack spacing={2.5}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: '1fr 1fr' }, gap: 2 }}>
        <Paper sx={{ p: 2.5, height: 380 }}>
          <Typography variant="h6" fontWeight={900}>{isRtl ? 'الاتجاه الشهري لإجمالي المؤشرات' : 'Monthly trend'}</Typography>
        ...
        <Paper sx={{ p: 2.5, height: 380 }}>
          <Typography variant="h6" fontWeight={900}>{isRtl ? 'المستهدف السنوي مقابل المنجز' : 'Annual target vs achieved'}</Typography>
    ...
    {tab === 1 && summary && <Stack spacing={2}>
      ...
      <Paper sx={{ overflow: 'hidden' }}>
        ...
        <TableContainer sx={{ maxHeight: '68vh' }}><Table stickyHeader size="small" sx={{ minWidth: 1450 }}><TableHead><TableRow>
          <TableCell sx={{ minWidth: 105 }}>{isRtl ? 'التاريخ' : 'Date'}</TableCell>
          ...
          {days.map(day => {
            ...
            return <TableRow key={day} hover>
              ...
              <TableCell>{getDayName(day)}</TableCell>
              {summary.indicators.map(indicator => {
                const key = `${dateValue}|${indicator.id}`;
          ...
          <TableRow sx={{ bgcolor: 'rgba(15,111,109,.08)' }}>
            ...
            {summary.indicators.map(indicator => <TableCell key={indicator.id} align="center" sx={{ fontWeight: 950 }}>{indicatorMonthTotals[indicator.id] || 0}</TableCell>)}
            <TableCell align="center" sx={{ fontWeight: 950 }}>{Object.values(indicatorMonthTotals).reduce((sum, value) => sum + value, 0)}</TableCell>
            <TableCell colSpan={3} />
          </TableRow>
        </TableBody></Table></TableContainer>
      </Paper>
    ...
    {tab === 2 && summary && <Paper sx={{ p: 2 }}><TableContainer><Table size="small"><TableHead><TableRow>
      <TableCell>{isRtl ? 'المؤشر' : 'Indicator'}</TableCell>
      {monthNames.map(name => <TableCell key={name} align="center">{name}</TableCell>)}
      <TableCell align="center">{isRtl ? 'السنوي' : 'Annual'}</TableCell>
    </TableRow></TableHead><TableBody>
      {summary.indicators.map(indicator => <TableRow key={indicator.id}><TableCell sx={{ minWidth: 220, fontWeight: 800 }}>{isRtl ? indicator.name_ar : indicator.name_en}</TableCell>{indicator.monthly.map((value, index) => <TableCell key={index} align="center">{value}</TableCell>)}<TableCell align="center" sx={{ fontWeight: 900 }}>{indicator.annual_achieved}</TableCell></TableRow>)}
    </TableBody></Table></TableContainer></Paper>}

    {tab === 3 && summary && <Paper sx={{ p: 2 }}><TableContainer><Table size="small"><TableHead><TableRow>
      <TableCell>{isRtl ? 'المؤشر' : 'Indicator'}</TableCell>
      {[1,2,3,4].flatMap(value => [<TableCell key={`t${value}`} align="center">Q{value} {isRtl ? 'مستهدف' : 'Target'}</TableCell>, <TableCell key={`a${value}`} align="center">Q{value} {isRtl ? 'منجز' : 'Done'}</TableCell>, <TableCell key={`p${value}`} align="center">%</TableCell>])}
      <TableCell>{isRtl ? 'سنوي' : 'Annual'}</TableCell>
    </TableRow></TableHead><TableBody>
      {summary.indicators.map(indicator => <TableRow key={indicator.id}><TableCell sx={{ fontWeight: 800 }}>{isRtl ? indicator.name_ar : indicator.name_en}</TableCell>{indicator.quarterly.flatMap(row => [<TableCell key={`t${row.quarter}`} align="center">{row.target ?? '—'}</TableCell>,<TableCell key={`a${row.quarter}`} align="center">{row.achieved}</TableCell>,<TableCell key={`p${row.quarter}`} align="center">{percentage(row.percentage)}</TableCell>])}<TableCell><b>{indicator.annual_achieved}</b> / {indicator.annual_target ?? '—'}</TableCell></TableRow>)}
    </TableBody></Table></TableContainer></Paper>}
    ...
    {tab === 7 && <Stack spacing={2.5}>
      ...
      {canEdit && <Paper sx={{ p: 2.5 }}>
        ...
        <Box sx={{ mt: 2, display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3,1fr)' }, gap: 1.5 }}>
          <TextField label={isRtl ? 'الوثيقة / المرجع' : 'Document / Reference'} value={documentForm.title} onChange={event => setDocumentForm(value => ({ ...value, title: event.target.value }))} />
          <TextField select label={isRtl ? 'الحالة' : 'Status'} value={documentForm.status} onChange={event => setDocumentForm(value => ({ ...value, status: event.target.value as ReferenceDocument['status'] }))}><MenuItem value="available">{isRtl ? 'متوفر' : 'Available'}</MenuItem><MenuItem value="unavailable">{isRtl ? 'غير متوفر' : 'Unavailable'}</MenuItem><MenuItem value="not_applicable">{isRtl ? 'لا ينطبق' : 'Not applicable'}</MenuItem></TextField>
          <TextField required={documentForm.status !== 'available'} label={isRtl ? 'السبب' : 'Reason'} value={documentForm.reason} onChange={event => setDocumentForm(value => ({ ...value, reason: event.target.value }))} />
          <TextField label={isRtl ? 'المصدر / المرجع' : 'Source'} value={documentForm.source} onChange={event => setDocumentForm(value => ({ ...value, source: event.target.value }))} />
          <TextField label={isRtl ? 'الجهة المصدرة' : 'Issuing authority'} value={documentForm.issuing_authority} onChange={event => setDocumentForm(value => ({ ...value, issuing_authority: event.target.value }))} />
          <TextField label={isRtl ? 'رقم الإصدار' : 'Version'} value={documentForm.version_number} onChange={event => setDocumentForm(value => ({ ...value, version_number: event.target.value }))} />
          <CalendarDateField label={isRtl ? 'تاريخ الإصدار' : 'Issue date'} value={documentForm.issue_date} onChange={value => setDocumentForm(current => ({ ...current, issue_date: value }))} />
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(min(100%,470px),1fr))', gap: 2 }}>
        {documents.map(doc => <Paper key={doc.id} sx={{ p: 2.2, borderTop: '4px solid', borderTopColor: doc.status === 'available' ? 'success.main' : doc.status === 'unavailable' ? 'error.main' : 'grey.500' }}>
          <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={1}>
            <Box sx={{ minWidth: 0 }}><Typography variant="h6" fontWeight={900}>{doc.title}</Typography><Typography variant="caption" color="text.secondary">{[doc.source, doc.issuing_authority].filter(Boolean).join(' · ') || (isRtl ? 'بدون مصدر مسجل' : 'No source recorded')}</Typography></Box>
            <Chip size="small" color={doc.status === 'available' ? 'success' : doc.status === 'unavailable' ? 'error' : 'default'} label={doc.status === 'available' ? (isRtl ? 'متوفر' : 'Available') : doc.status === 'unavailable' ? (isRtl ? 'غير متوفر' : 'Unavailable') : (isRtl ? 'لا ينطبق' : 'Not applicable')} />
          </Stack>
          <Divider sx={{ my: 1.4 }} />
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: .8 }}>
            <Typography variant="body2"><b>{isRtl ? 'العدد:' : 'Count:'}</b> {doc.document_count || 1}</Typography>
            <Typography variant="body2"><b>{isRtl ? 'الإصدار:' : 'Version:'}</b> {doc.version_number || '—'}</Typography>
            <Typography variant="body2"><b>{isRtl ? 'تاريخ الإصدار:' : 'Issue date:'}</b> {doc.issue_date || '—'}</Typography>
            <Typography variant="body2"><b>{isRtl ? 'آخر مراجعة:' : 'Last review:'}</b> {doc.last_review_date || '—'}</Typography>
            <Typography variant="body2" sx={{ gridColumn: '1 / -1' }}><b>{isRtl ? 'المسؤول:' : 'Responsible:'}</b> {doc.responsible_person || '—'}</Typography>
            {doc.reason && <Typography variant="body2" sx={{ gridColumn: '1 / -1' }}><b>{isRtl ? 'السبب:' : 'Reason:'}</b> {doc.reason}</Typography>}
            {doc.notes && <Typography variant="body2" color="text.secondary" sx={{ gridColumn: '1 / -1' }}>{doc.notes}</Typography>}
          </Box>
          {(doc.evidence_links || []).length > 0 && <Box sx={{ mt: 1.3 }}><Typography variant="subtitle2" fontWeight={900}>{isRtl ? 'روابط إثبات خارجية' : 'External evidence links'}</Typography>{(doc.evidence_links || []).map((link, index) => <Typography key={index} component="a" href={link} target="_blank" rel="noreferrer" variant="caption" sx={{ display: 'block', color: 'primary.main', overflowWrap: 'anywhere' }}>{link}</Typography>)}</Box>}
          <EvidenceAttachmentManager ownerType="reference_document" ownerId={doc.id} canEdit={canEdit} isRtl={isRtl} initialAttachments={doc.attachments} />
        </Paper>)}
        {documents.length === 0 && <EmptyState text={isRtl ? 'لا توجد وثائق أو مراجع مسجلة.' : 'No documents registered.'} />}
      </Box>
    </Stack>}
  </Box>;
}
